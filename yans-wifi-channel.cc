      ...
      if (sender != (*i))
        {
          ...
          // calc txgain
          double txGain = 0;
          //Ptr<AntennaModel> sendAnt = sender->GetObject<AntennaModel>();
          Ptr<AntennaModel> sendAnt = sender->GetAntenna();
          
          if(sendAnt != 0)
          {          
        	  
                  Angles txAngles (receiverMobility->GetPosition (), senderMobility->GetPosition ());
                  txGain = sendAnt->GetGainDb (txAngles);
                  //txGain = 0;
          }
          ...
          // calc rxGain
          double rxGain = 0;
          //Ptr<AntennaModel> recvAnt = (*i)->GetObject<AntennaModel>();
          Ptr<AntennaModel> recvAnt = (*i)->GetAntenna();
          if(recvAnt != 0)
          {
                  Angles rxAngles (senderMobility->GetPosition (), receiverMobility->GetPosition ());
        	  rxGain = recvAnt->GetGainDb (rxAngles);
                  //rxGain = 0;
        	  rxPowerDbm+=rxGain;
          }
          NS_LOG_DEBUG ("propagation: txPower=" << txPowerDbm << "dbm, txGain=" << txGain << "dbm, rxGain=" << rxGain << "dbm, rxPower=" << rxPowerDbm << "dbm, " << "distance=" << senderMobility->GetDistanceFrom (receiverMobility) << "m, delay=" << delay);
          //modification by jerry ends
